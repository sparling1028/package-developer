...
def get_local_names(text):
    function_name = get_function_name(text)
    signature = get_signature(text)
    body = text.split(signature)[1]
    matches = {re.search(r'\w+', string)
                  for string in 
                  __remove_square_parentheticals_and_quoted_things__(signature).split(',')
                 }
    parameters = {x.group() for x in {x for x in matches if x != None}}
    nonlocals = get_explicit_nonlocals(text)
    things_to_ignore = parameters.union(nonlocals)
    otherwise_bound_names = set()
    for line in text.split(signature)[1].splitlines():
        match = re.match(r'(\w+)\s*=[^=]', line.strip())
        if bool(match) == True:
            otherwise_bound_names.add(match.groups()[0])
    return otherwise_bound_names.difference(nonlocals).union(parameters)


def __remove_square_parentheticals_and_quoted_things__(text):
    text = remove_quoted_things(text)
    while True:
        text = re.sub(r'\[[^\[\]]+\]', '[]', text, flags=re.MULTILINE+re.DOTALL)
        if bool(re.search(r'\[[^\[\]]+\]', text, flags=re.MULTILINE+re.DOTALL)) == False:
            break
    return text


def get_explicit_nonlocals(text):
    function_name = get_function_name(text)
    signature = get_signature(text)
    body = text.split(signature)[1]
    ...
    for line in body.splitlines():
        ...
        if line.split() == []:
            pass
        elif line.split()[0] in {'global', 'nonlocal'}:
            explicit_nonlocals = explicit_nonlocals.union(set(line.replace(',', ' ').split()[1:]))
    return explicit_nonlocals


def get_function_name(text):
    return re.search(r'(?<=def )\w+', text).group()

def get_signature(text):
    beginning_of_signature = re.search(r'\(', text).start()
    end_of_signature = re.search(r'\):', text, re.M).end()
    signature = text[beginning_of_signature: end_of_signature]
    return signature
...
#  method group: attribute manipulations
    ...
    def add_module_attribute(self, name, attribute_code, type_function=lambda x: x):
        ...
        path = os.path.join(self.__class_root__, "shared_attributes.attributes")
        self.__save_module_directory__()
        pseudo_imported_names[f"{self.target_module.__name__}.{self.target_class.__name__}"][name] = eval(name)
        return self
...
#  method group: method and function manipulations
    ...
    def add_pseudo_imported_names(self, text):
        for name in (
                ...
                .difference(get_local_names(text))
                .difference({get_function_name(text)})
                     ):
            text = re.sub(
                pattern = fr"(?<=[^\w\.]){name}(?=[^\w])", 
    ...
    def commit_function(self, function, group_name=None, function_name=None, function_text=None):
        ...
        altered_function_text = self.add_pseudo_imported_names(function_text)
        exec(altered_function_text, globals())
        # set to module
        ...
        self.__save_module_directory__()
        # add_to_pseudo_imported_names
        pseudo_imported_names[f"{self.target_module.__name__}.{self.target_class.__name__}"][function_name] = function
        return self
